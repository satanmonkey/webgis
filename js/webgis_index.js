...
function testheatmap(viewer)
{
    ...
    if($.webgis.config.map_backend === 'cesium')
    {
        ...
        //DrawHeatMapCylender(viewer, points);
    }
    if($.webgis.config.map_backend === 'leaflet')
    {    
        var cfg = {
...
function DrawHeatMapLegend(viewer, list)
{
    var get_max_min = function(list)
    {
        var ret = [99999, 0];
    ...
    {
        var ret = [];
        _.forIn($.webgis.mapping.heat_map_gradient_stops, function(v, k)
        {
    ...
    var get_alpha = function(range, gradient_list, v)
    {
        ...
        _.forEach(_.range(1, gradient_list.length), function(i)
        {
    ...
    var create_legend_div = function(range)
    {
        ...
        $( '<div id="heatmaplegend">\
                ...
                <div class="heatmaplegend_scale_high1">\
                </div>\
                <div class="heatmaplegend_title_high">\
                </div>\
                <div class="heatmaplegend_title_high1">\
                </div>\
                <div class="heatmaplegend_scale_medium">\
                </div>\
                <div class="heatmaplegend_scale_medium1">\
                </div>\
                <div class="heatmaplegend_title_medium">\
                </div>\
                <div class="heatmaplegend_title_medium1">\
                </div>\
                <div class="heatmaplegend_scale_low">\
                </div>\
                <div class="heatmaplegend_title_low">\
                </div>\
            <div>'
        ).appendTo('body');
        var medium = Math.floor((range[0] + range[1])/2);
        $('#heatmaplegend .heatmaplegend_title_high').html(range[1]);
        $('#heatmaplegend .heatmaplegend_title_high1').html(Math.floor((range[1] + medium)/2));
        $('#heatmaplegend .heatmaplegend_title_medium').html(medium);
        $('#heatmaplegend .heatmaplegend_title_medium1').html(Math.floor((range[0] + medium)/2));
        $('#heatmaplegend .heatmaplegend_title_low').html(0);//(range[0]);
    };
    var range = get_max_min(list);
    //var gradient_list = get_gradient_list();
    //var colorlist = _.map(list, function(item){
    //    var o = {};
    //    o.count = item.count;
    //    o.color =  get_gradient_stop(range, gradient_list, item.count);
    //    return o;
    //});
    //console.log(colorlist);
    create_legend_div(range);
}
function DrawHeatMapCylender(viewer, hid, list, height_magnifier)
{
    var get_max_min = function(list)
    {
        var ret = [99999, 0];
...
function ShowAntiBirdStatisticsDialog(viewer)
{
    ...
    $('#form_anti_bird_statistics_heatmap').webgisform('setdata',{
        slider_distinguish_speed:[3,15],
        start_date:moment('2015-06-10').local().format('YYYYMMDD'),
...
function BuildAntiBirdStatisticsForm(viewer)
{
    ...
    var build = function(towers) {
        ...
        $('#form_anti_bird_statistics_chart').webgisform('setdata', {
            type: '',
            start_date: moment('2015-06-10 00:00').local().format('YYYYMMDD HH:mm'),
...
function BuildAntiBirdInfoChartForm(imei)
{
    ...
    $('#form_anti_bird_info_chart').webgisform('setdata',{
        type:'D',
        start_date:moment('2015-06-10').local().format('YYYYMMDD'),
        end_date:moment().local().format('YYYYMMDD')});
    ...
    if(dict.speed && dict.speed.length === 2 && dict.start && dict.start.length>0 && dict.end && dict.end.length>0)
    {
        ...
        $.get(url, {}, function( data1 ){
            ...
            if(_.isArray(data1))
            {
                ...
                if($.webgis.config.map_backend === 'cesium')
                {
                    //DrawHeatMapCylender(viewer, hid, data1, 200);
                    DrawHeatMapLegend(viewer, data1);
                }
            }
            ShowProgressBar(false);
        }, 'json');
