...
var ArcGisMapServerImageryProvider = function ArcGisMapServerImageryProvider(options) {
    ...
    this._customMaximumLevel = Cesium.defaultValue(options.maximumLevel,  undefined);
    this._customMinimumLevel = Cesium.defaultValue(options.minimumLevel,  undefined);
    ...
    this._format = Cesium.defaultValue(options.format, 'jpg');
    ...
    function metadataSuccess(data) {
        ...
        if (!that._useTiles || !Cesium.defined(tileInfo)) {
            that._tileWidth = 256;
            ...
            if(that._customMaximumLevel != undefined)
            {
                that._maximumLevel = that._customMaximumLevel;
                //console.log(that._maximumLevel);
            }
        } else {
            ...
            if (Cesium.defined(data.fullExtent)) {
                ...
                var sw = projection.unproject(new Cesium.Cartesian2(data.fullExtent.xmin, data.fullExtent.ymin));
                var ne = projection.unproject(new Cesium.Cartesian2(data.fullExtent.xmax, data.fullExtent.ymax));
    ...
    function requestMetadata() {
        var metadata = Cesium.jsonp(that._url, {
            parameters : {
                f : 'json'
            },
            proxy : that._proxy
...
function buildImageUrl(imageryProvider, x, y, level) {
    ...
    if (imageryProvider._useTiles) {
        ...
        url = imageryProvider._url + '/exportImage?';
        url += 'bbox=' + bbox;
        //url += '&bboxSR=4326&size=256%2C256&imageSR=4326&format=png&transparent=true&f=image';
        url += '&bboxSR=4326&size=256%2C256&imageSR=4326&format=' + imageryProvider._format + '&transparent=true&f=image';
    }
...
Cesium.defineProperties(ArcGisMapServerImageryProvider.prototype, {
    /**
...
//ArcGisMapServerImageryProvider.prototype.getLayer = function() {
	//var ret, i;
	//if(this.viewer)
	//{
...
//ArcGisMapServerImageryProvider.prototype.destroy = function() {
