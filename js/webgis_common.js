...
$.webgis.mapping.style_mapping = {
	'point_tower' :		{icon_img:'img/features/powerlinepole.png', color:[128, 128, 255, 255],outlineColor:[0, 0, 0, 255], outlineWidth:1, pixelSize:10, labelFillColor:[128, 255, 0, 255], labelOutlineColor:[255, 255, 255, 255], labelScale: 0.6},
	...
	var methods = 
	{
		...
		setdata : function(data)
		{
			...
			for(var k in fields)
			{
				...
				var typ = fields[k].type;
				var editor = fields[k].editor;
				if(typ.indexOf( 'button_') > -1 || typ === 'label' || typ === 'file')
				{
					continue;
				}
				else if(typ === 'icon')
		...
		getdata:function()
		{
			...
			for(var k in fields)
			{
				var id = fields[k]['id'];
				var typ = fields[k]['type'];
				if(typ === 'button' || typ === 'label' || typ === 'file')
				{
					continue;
				}
				else if(typ === 'icon')
